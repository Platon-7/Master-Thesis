...
def collect(task, key):
    ...
    agg={}
    for d in sorted(glob.glob(f"{RD}/onlinecal_demoanchor_{CKPT}_{task}_*_755_*")):
...
for col,(task,tlabel) in enumerate([("coffeepush","Coffee-Push"),("boxclose","Box-Close")]):
    ...
    ax.plot(xs,mean,color=GREEN,lw=2.6,label="GT success (mean of 3 seeds)")
...
fig.suptitle("Gate + demo-anchor threshold  ·  run2  ·  3 seeds", fontsize=12.5)
fig.tight_layout()
out="/shared/home/PKA4388/Master-Thesis/post-training-analysis/reward-model-study/deck/fig_bestcase_demoanchor_run2.png"
