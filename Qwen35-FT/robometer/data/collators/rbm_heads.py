...
def should_compute_progress(
    ...
    # predict partial success for roboreward trajectories not preference only
        ...
        return 1.0
    # Our keyframe dataset ships curated per-frame ordinal labels on failures (`frame_labels`
    # on the loader output → `target_progress` via rubric in the sampler). So failures from
    # robometer_frames_* ARE supervised, unlike the generic "failure" trajectories that get
    # masked out below.
    elif data_source is not None and data_source.startswith("robometer_frames_"):
        return 1.0
...
class RBMBatchCollator(BaseCollator):
    ...
    def _process_progress_batch(self, progress_samples: list[ProgressSample]) -> dict[str, torch.Tensor]:
        ...
        # Per-sample count of in-context demo frames that sit BEFORE the query frames in the prompt.
        # 0 when the sample has no context_trajectory, otherwise the number of demo frames (typically 16).
        # Consumed downstream by the trainer to drop demo positions from per-frame outputs before
        # progress-head loss is computed.
        demo_frames_counts: list[int] = []
        ...
        for sample in progress_samples:
            ...
            # --- ICL branch -------------------------------------------------------------------
            # If the sampler attached a successful demo trajectory, prepend its frames to the prompt
            # so the model sees `[demo frames] <|demo_end|> [query frames]`. Per-frame progress
            # labels still reference query frames only.
            context_traj = sample.context_trajectory
            if context_traj is not None:
                demo_pil_frames = convert_frames_to_pil_images(context_traj.frames, context_traj.frames_shape)
                demo_video_field, demo_content_extras = self._prepare_frames_for_conversation(
                    demo_pil_frames, prefix="tmp_progress_demo"
                )
                ...
                prompt = (
                    ...
                    "Predict the task progress at each frame of the trajectory to be evaluated (not the demonstration) "
                    "— a float between 0 and 1, where 0 is the starting state and 1 is when the task is completed. "
                    "If the robot is not performing the same task, predict 0 progress."
                )
                content_list = [
                    {"type": "text", "text": prompt},
                    {"type": "text", "text": "Successful demonstration:"},
                ]
                self._add_vision_content_to_list(content_list, demo_video_field, demo_content_extras)
                content_list.append({"type": "text", "text": "<|demo_end|>"})
                content_list.append({"type": "text", "text": "Trajectory to evaluate:"})
        ...
        batch_inputs["demo_frames_count"] = torch.tensor(demo_frames_counts, dtype=torch.int32)
        return batch_inputs
    ...
    def _process_preference_batch(self, preference_samples: list[PreferenceSample]) -> dict[str, torch.Tensor]:
        ...
        # Per-sample count of in-context demo frames that sit BEFORE the trajectory-A frames.
        # 0 when no ICL demo is attached; otherwise the demo's frame count (typically 16).
        demo_frames_counts: list[int] = []
        ...
        for i, sample in enumerate(preference_samples):
            ...
            # --- ICL branch -------------------------------------------------------------------
            # If the sampler attached a successful demo trajectory, prepend its frames before
            # Trajectory A so the model sees `[demo] <|demo_end|> [A] <|split_token|> [B] <|pref_token|>`.
            # The demo is shared context for both A and B; loss is computed on A/B only.
            context_traj = sample.context_trajectory
            if context_traj is not None:
                demo_pil_frames = convert_frames_to_pil_images(context_traj.frames, context_traj.frames_shape)
                demo_video_field, demo_content_extras = self._prepare_frames_for_conversation(
                    demo_pil_frames, prefix="tmp_pref_demo"
                )
                demo_frames_counts.append(len(demo_pil_frames))

                content_list = [
                    {"type": "text", "text": prompt},
                    {"type": "text", "text": "Successful demonstration of this task:"},
                ]
                self._add_vision_content_to_list(content_list, demo_video_field, demo_content_extras)
                content_list.append({"type": "text", "text": "<|demo_end|>"})
                content_list.append({"type": "text", "text": "This is Trajectory A. "})
            else:
                demo_frames_counts.append(0)
