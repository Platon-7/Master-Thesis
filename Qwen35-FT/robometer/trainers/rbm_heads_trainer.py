...
from robometer.data.samplers.stratified_warmup import (
...
class RBMHeadsTrainer(Trainer):
    ...
    def get_train_dataloader(self) -> DataLoader:
        """Override to install a stratified-warmup batch sampler when configured.

        Two-phase data scheduling for the LoRA bake-off:
          * step <  data.warmup_steps : batches drawn from failure samples whose
        ...
        # Walk through any wrappers (RepeatedDataset → RBMDataset → HF dataset).
        underlying = getattr(self.train_dataset, "dataset", self.train_dataset)
        hf_ds = getattr(underlying, "dataset", underlying)
        try:
            data_sources = hf_ds["data_source"]
            quality_labels = hf_ds["quality_label"]
        except (KeyError, TypeError) as e:
            raise RuntimeError(
                "stratified_batch_balance=True requires the train dataset to expose "
                "'data_source' and 'quality_label' columns; could not read them: " + str(e)
            )

        warmup_idx: List[int] = []
        failure_idx: List[int] = []
        success_idx: List[int] = []
        for i, (src, lbl) in enumerate(zip(data_sources, quality_labels)):
            is_warmup = bool(src) and "warmup" in src
            if is_warmup:
                warmup_idx.append(i)
            elif lbl == "successful":
        ...
        self.add_callback(WarmupStepSyncCallback(batch_sampler))
    ...
    def _get_optimizer_stats(self):
        ...
        # Top-N per-parameter-name grad/param norms removed (Apr-30): each LoRA layer's name
        # was baked into the metric key, producing ~336 sparse, mostly-empty wandb series
        # that nobody inspected. The high-level summary scalars above (preclip_grad_norm,
        # _max, _min, param_norm, _max, _min, exp_avg_norm_*) already cover "is anything
        # weird going on" at the run level. To bring back per-layer detail in a debug pass,
        # iterate `self.model.named_parameters()`, sort by .grad.data.norm(2).item(), and
        # log the top-5 with rank-only metric keys (no name in the key) so the series stay
        # stable across steps.
    ...
    def _log_metadata(self):
        ...
        # Log10 mirror of selected training metrics so wandb workspace gets a log-scale
        # twin panel for the loss curves. Picked metrics:
        #   - train/loss          → train/loss_log10
        #   - train/prog_loss     → train/prog_loss_log10
        #   - train/spearman_corr → train/spearman_corr_log10  (clamped to >=1e-8 to handle <=0)
        #   - train_ds_prog_loss/<source>     → train_ds_prog_loss_log10/<source>
        #   - train_ds_spearman_corr/<source> → train_ds_spearman_corr_log10/<source>
        # Spearman is bounded [-1, 1] so log-scale is mostly informative when corr is small;
        # we still emit the variant for parity with the user's request.
        _LOG10_TARGETS = ("train/loss", "train/prog_loss", "train/spearman_corr",
                          "train/success_loss", "train/success_auprc")
        _LOG10_DS_PREFIXES = ("train_ds_prog_loss/", "train_ds_spearman_corr/",
                              "train_ds_success_loss/", "train_ds_success_auprc/")
        log10_extras: Dict[str, float] = {}
        import math as _math
        for key, val in log_data.items():
            if key in _LOG10_TARGETS or any(key.startswith(p) for p in _LOG10_DS_PREFIXES):
                # Clamp to >= 1e-8 so log10 is finite even on negative or zero spearman.
                v = max(float(val), 1e-8)
                # Insert _log10 before the trailing /<source> path segment if present, else
                # append to the metric name. Result: parallel namespace, easy to find in wandb.
                if "/" in key and key.startswith(("train_ds_",)):
                    base, source = key.split("/", 1)
                    log10_extras[f"{base}_log10/{source}"] = _math.log10(v)
                else:
                    log10_extras[f"{key}_log10"] = _math.log10(v)
        log_data.update(log10_extras)
    ...
    def _setup_eval_dataset(self, eval_type, eval_dataset):
        ...
        # The drift-sniff assertion (`data.icl_min_coverage`) is a TRAINING-time guard
        # against HF-vs-pair_index ID mismatch. Eval samplers below get a per-split pair
        # index (see derivation just below) which legitimately has different partner counts
        # — disable the strict floor for eval. The training-pool sampler still enforces it.
        eval_cfg.icl_min_coverage = 0.0

        # Per-split pair-index plumbing for dual-ICL eval. The global `data.pairs_index_path`
        # is the TRAIN pair index (used during training). Eval splits have their own pair
        # indices on disk (`pairs_index_<split>.jsonl`); without overriding, eval samplers
        # read the train index and find 0% partner overlap with their HF episode IDs →
        # every dual-ICL pass collapses to ICL-off. Derive the per-split path from the
        # dataset name (`robometer_frames_<split>` → `pairs_index_<split>.jsonl`) and override.
        from pathlib import Path as _Path
        primary_ds = eval_dataset[0] if isinstance(eval_dataset, list) else eval_dataset
        if (eval_cfg.pairs_index_path
                and primary_ds.startswith("robometer_frames_")
                and getattr(eval_cfg, "use_icl", False)):
            split_suffix = primary_ds[len("robometer_frames_"):]   # e.g. "eval_droid"
            base_dir = _Path(eval_cfg.pairs_index_path).parent
            derived = base_dir / f"pairs_index_{split_suffix}.jsonl"
            if derived.is_file():
                logger.info(f"  [eval pair-index] {primary_ds} → {derived}")
                eval_cfg.pairs_index_path = str(derived)
            else:
                logger.warning(
                    f"  [eval pair-index] {primary_ds}: derived path {derived} does not "
                    f"exist; falling back to global pairs_index_path. ICL-on/off panels "
                    f"will be identical for this split."
                )
    ...
    def _process_batch_progress_eval(self, batch, eval_type):
        ...
        # CORN head (losses.md §Loss 1) emits 4 cumulative-threshold logits per frame
        # (shape [B, T, 4]). Downstream eval pipelines (compile_results.run_*_eval) assume
        # a scalar progress per frame in continuous mode, or [seq, num_bins] in discrete
        # mode. CORN matches neither — decode to scalar expected progress in [0, 1] at the
        # source so all three eval types (policy_ranking, confusion_matrix, reward_alignment)
        # see a uniform [B, T] tensor and the existing continuous-mode branches handle it.
        # Decoding: P(success ≥ k) = σ(z_k); E[y] − 1 = Σ_k σ(z_k); normalised by 4 → [0, 1].
        progress_head_mode = getattr(self.config.model, "progress_head_mode", "c51")
        # Preserve the raw 4-logit CORN output so post-hoc analysis can reconstruct any of
        # the per-threshold probabilities — in particular σ(z_{t,5}) which losses.md §Loss 1
        # designates as P(success at frame t) for L1. Without this, only the source-decoded
        # expected-progress scalar (1/4) Σ_k σ(z_k) is recoverable from eval JSONs.
        progress_pred_raw = None
        if str(progress_head_mode).lower() == "corn" and progress_pred.dim() == 3 and progress_pred.shape[-1] == 4:
            progress_pred_raw = progress_pred  # [B, T, 4] cumulative-threshold logits
            progress_pred = torch.sigmoid(progress_pred).sum(dim=-1) / 4.0

        # Gather everything
        progress_pred = self.accelerator.gather_for_metrics(progress_pred)
        if progress_pred_raw is not None:
            progress_pred_raw = self.accelerator.gather_for_metrics(progress_pred_raw)
        ...
        for i in range(len(progress_pred)):
            ...
            if progress_pred_raw is not None:
                # Raw [T, 4] CORN cumulative-threshold logits; post-hoc analysis can
                # extract σ(z_{t,5}) = sigmoid(progress_pred_raw[..., 3]) per losses.md.
                sample_result["progress_pred_raw"] = t2n(progress_pred_raw[i])
    ...
    def _run_custom_evaluations(self, eval_step=None):
        ...
        for eval_type in eval_types:
            ...
            # Dual-ICL eval for policy_ranking: when configured, run each split twice —
            # once with icl_prob temporarily forced to 1.0 (ICL-on, every paired sample
            # gets a demo) and once with icl_prob forced to 0.0 (ICL-off). Metric keys
            # gain `_iclon` / `_icloff` suffixes so wandb sees them as separate scalars.
            # The training-time data.icl_prob (0.5 coin-flip) is unchanged outside of
            # the eval window — we mutate-and-restore around each pass.
            dual_icl_active = (
                eval_type == "policy_ranking"
                and bool(getattr(self.config.custom_eval, "policy_ranking_dual_icl", False))
                and bool(getattr(self.config.data, "use_icl", False))
            )
            if dual_icl_active:
                icl_passes = [("iclon", 1.0), ("icloff", 0.0)]
            else:
                icl_passes = [(None, None)]

            with _timer(f"time/eval_type/{eval_type}", timing_raw=self.timing_raw):
                for eval_dataset in eval_datasets_name:
                    for pass_tag, pass_prob in icl_passes:
                        # Temporarily override icl_prob for this pass; restore afterwards.
                        if pass_prob is not None:
                            _saved_icl_prob = self.config.data.icl_prob
                            self.config.data.icl_prob = pass_prob
                        try:
                            eval_metrics, ds_name = self._run_single_eval_dataset(eval_type, eval_dataset, eval_step)
                        finally:
                            if pass_prob is not None:
                                self.config.data.icl_prob = _saved_icl_prob

                        if pass_tag is not None:
                            # Suffix scalar metrics so iclon/icloff are distinguishable in
                            # the flattened wandb dict. Non-scalar payloads (per-task tables,
                            # etc.) still merge under the same eval_type key — only the last
                            # pass wins for those, which is fine for downstream artifacts.
                            tagged = {}
                            for k, v in eval_metrics.items():
                                if isinstance(v, (int, float)):
                                    tagged[f"{k}_{pass_tag}"] = v
                                else:
                                    tagged[k] = v
                            existing = metrics[ds_name].get(eval_type, {}) or {}
                            existing.update(tagged)
                            metrics[ds_name][eval_type] = existing
                        else:
                            metrics[ds_name][eval_type] = eval_metrics

                    # Store timing for this eval_dataset (last pass wins; both passes are
                    # ~equal time so this is fine)
                    dataset_for_mapping = eval_dataset[0] if isinstance(eval_dataset, list) else eval_dataset
            ...
            for eval_type, metric in eval_type_metric.items():
                ...
                for k, v in metric.items():
                    if not isinstance(v, (int, float)):
                        continue
                    # Group by metric type so wandb gets one collapsible section per metric
                    # ("eval_p_rank/kendall_avg" with 4 dataset panels) instead of 96 sibling
                    # scalars under a flat eval_p_rank/. If dual-eval added an `_iclon`/`_icloff`
                    # suffix on k, we strip it and append to the dataset path segment so the
                    # iclon/icloff panels for the same dataset appear adjacent.
                    icl_suffix = ""
                    metric_key = k
                    for sfx in ("_iclon", "_icloff"):
                        if metric_key.endswith(sfx):
                            icl_suffix = sfx
                            metric_key = metric_key[: -len(sfx)]
                            break
                    metric_name = f"eval_{eval_type_short}/{metric_key}/{ds_name}{icl_suffix}"
    ...
    def _compute_success_loss_helper(
        ...
        success_loss_type = getattr(self.config.loss, "success_loss_type", "bce_balanced").lower()
        combined_mask_index = combined_mask.bool()

        if success_loss_type == "bce_asymmetric":
            # Chris' asymmetric BCE (losses.md §Loss 2): raw per-element BCE, then
            #   L_succ = mean(bce[neg]) + lambda · mean(bce[pos])
            # with an empty-positives guard. The lambda < 1 damps the positive class so the
            ...
            loss = loss * combined_mask  # zero out masked-out frames

            mask_neg_bool = (success_labels == 0) & combined_mask_index
            mask_pos_bool = (success_labels == 1) & combined_mask_index

            # Guard each .mean() against empty slices (all-failures batch → no positives; or
            # the entire batch masked out → no negatives either).
            ...
            # Book-keeping shared with the balanced branch: sample_weights is only used for
            # logging downstream, so report the per-class multiplier (1 on negatives, lambda
            # on positives). success_loss_weight logs the lambda itself.
            ...
            # Default: Robometer's existing class-balanced BCE.
        ...
        # False-positive / false-negative rates over the masked positions.
        # FP rate (false declarations of success on a non-success frame) is the "reward
        # hacking" signal we care about for the bake-off — losses.md §Evaluation primary metrics.
        valid = combined_mask.bool()
        fp_count = ((success_preds == 1.0) & (success_labels == 0.0) & valid).sum().float()
        fn_count = ((success_preds == 0.0) & (success_labels == 1.0) & valid).sum().float()
        fp_rate = (fp_count / (num_negatives + 1e-8)) if num_negatives > 0 else torch.tensor(0.0, device=success_logits.device, dtype=torch.float32)
        fn_rate = (fn_count / (num_positives + 1e-8)) if num_positives > 0 else torch.tensor(0.0, device=success_logits.device, dtype=torch.float32)
    ...
    def _compute_progress_loss_helper(
        ...
        masked_correct = None
        masked_mae = None  # per-frame ordinal MAE — populated by discrete-family branches only

        # Set loss function based on loss type.
        # The `c51_asymmetric` variant is Chris' asymmetric C51 (see losses.md §Loss 2). It reuses
        # the symmetric discrete path — same soft-CE targets, same reshape/mask/accuracy plumbing —
        # and multiplies the per-sample CE by a detached per-element weight that damps the
        # underestimation side by `loss.asymmetric_lambda`, so false positives (overestimation)
        ...
        if loss_type in ("discrete", "c51_asymmetric"):
            ...
            # ----- Chris' asymmetric weight (c51_asymmetric variant only) ---------------------
            ...
            if loss_type == "c51_asymmetric":
                ...
                if len(target_progress.shape) == 2:
                    # Hard class-index targets: scalar target = center of that bin.
                    p_t = bin_centers[target_bins.long()]  # [B, T]
                else:
                    # Soft C51 distribution targets (the default Robometer path): scalar target
                    # is the expected value of the target distribution.
            ...
            # Per-frame ordinal MAE in bin units. Captures off-by-one severity that binary
            # accuracy doesn't — e.g., predicting bin 0 when target is bin 9 is much worse
            # than predicting bin 8 when target is bin 9, but accuracy treats both as wrong.
            if len(target_bins_flat.shape) == 2:
                target_idx_flat = torch.argmax(target_bins_flat, dim=-1)  # [B*T]
            else:
                target_idx_flat = target_bins_flat  # [B*T] hard ints
            mae_flat = (pred_bins_flat - target_idx_flat).abs().float()
            masked_mae = (mae_flat * mask_flat).view(batch_size, seq_len)
        elif loss_type == "corn_asymmetric":
            # ===== CORN asymmetric loss (losses.md §Loss 1) =================================
            # Input shapes:
            #   progress_pred   : [B, T, 4]  — cumulative-threshold logits (k=2..5)
            #   target_progress : [B, T]     — decimals in [0, 1]
            # Steps:
            #   (a) decimal → integer ordinal y ∈ {1..5} via  y = round(4·p) + 1
            #   (b) ordinal → 4 cumulative bits b_k = 1[y ≥ k]  for k=2..5
            #   (c) per-threshold asymmetric BCE with α_k = 1 + c·(k-2), β_k = 1
            #   (d) sum over the 4 thresholds, apply mask, reduce.
            # ================================================================================
            if progress_pred.dim() != 3 or progress_pred.shape[-1] != 4:
                raise ValueError(
                    f"corn_asymmetric requires progress_pred of shape [B, T, 4]; got {progress_pred.shape}. "
                    f"Ensure model.progress_head_mode='corn' so the head emits 4 logits."
                )
            batch_size, seq_len = target_progress.shape

            # (a) decimal → ordinal y ∈ {1..5}
            y = (target_progress.float() * 4.0).round().long() + 1
            y = y.clamp(min=1, max=5)  # [B, T]

            # (b) cumulative bits b_k = 1[y ≥ k] for k = 2, 3, 4, 5
            thresholds = torch.arange(2, 6, device=y.device)  # [4]
            bits = (y.unsqueeze(-1) >= thresholds).float()     # [B, T, 4]

            # (c) numerically stable per-threshold BCE using logsigmoid
            log_sigma = F.logsigmoid(progress_pred)            # log σ(z_k)
            log_one_minus_sigma = F.logsigmoid(-progress_pred) # log(1 - σ(z_k))

            c = float(getattr(self.config.loss, "corn_asymmetric_c", 1.5))
            alpha_k = 1.0 + c * torch.arange(0, 4, device=progress_pred.device, dtype=progress_pred.dtype)
            # α = (1, 1+c, 1+2c, 1+3c) for k in {2,3,4,5}
            alpha_k = alpha_k.view(1, 1, 4)

            # β_k = uniform 1 by default per losses.md §Loss 1; override via
            # `loss.corn_pos_weights` (a list of 4 floats) when class imbalance on the
            # rare upper thresholds (P(y≥4)≈20%, P(y≥5)≈6%) needs counter-pressure on
            # the positive side of the BCE — see CORN_PRETRAIN_DEBUG.md.
            pos_weights_cfg = getattr(self.config.loss, "corn_pos_weights", None)
            if pos_weights_cfg is not None:
                if len(pos_weights_cfg) != 4:
                    raise ValueError(
                        f"loss.corn_pos_weights must have 4 entries (k=2..5); got {pos_weights_cfg}"
                    )
                beta_k = torch.tensor(
                    list(pos_weights_cfg), device=progress_pred.device, dtype=progress_pred.dtype
                ).view(1, 1, 4)
            else:
                beta_k = torch.ones(1, 1, 4, device=progress_pred.device, dtype=progress_pred.dtype)

            pos_term = beta_k * bits * log_sigma               # β_k · b · log σ
            neg_term = alpha_k * (1.0 - bits) * log_one_minus_sigma
            per_threshold_loss = -(pos_term + neg_term)        # [B, T, 4]
            per_frame_loss = per_threshold_loss.sum(dim=-1)    # [B, T]

            # Prepare mask [B, T] (same logic the discrete branch uses)
            if mask.shape[1] != seq_len:
                mask_expanded = mask.expand(batch_size, seq_len)
            else:
                mask_expanded = mask

            # (d) apply mask — keep the same shape contract as the other branches.
            masked_loss = per_frame_loss * mask_expanded       # [B, T]
            loss_per_sample = per_frame_loss

            # Accuracy: decode predicted ordinal and compare with target ordinal.
            sigma = torch.sigmoid(progress_pred)               # [B, T, 4]
            pred_bits = (sigma > 0.5).float()                  # [B, T, 4]
            y_hat = 1 + pred_bits.sum(dim=-1).long()           # [B, T] in {1..5}
            correct = (y_hat == y).float()                     # [B, T]
            masked_correct = correct * mask_expanded
            # Per-frame ordinal MAE in 1..5-level units (same scale as the CORN target).
            masked_mae = (y_hat - y).abs().float() * mask_expanded
            # Also carry num_bins for the downstream metrics-dict logic (ignored on the CORN path).
            num_bins = 5
        elif loss_type == "l1":
            loss_fn = F.l1_loss
        else:
        ...
        if loss_type not in ("discrete", "c51_asymmetric", "corn_asymmetric"):
            loss_per_sample = loss_fn(progress_pred.float(), target_progress.float(), reduction="none")
            masked_loss = loss_per_sample * mask

        # Build continuous views of prediction and target for the Spearman correlation metric.
        if loss_type in ("discrete", "c51_asymmetric"):
            ...
            # CORN decoding: expected ordinal E[y] = 1 + Σ_k σ(z_k), mapped to [0, 1]
            # via (E[y] − 1) / 4 = Σ_k σ(z_k) / 4.
            progress_pred_for_corr = torch.sigmoid(progress_pred).sum(dim=-1) / 4.0  # [B, T]
            target_progress_for_corr = target_progress  # already decimals in [0, 1]
        else:
        ...
        # Add progress accuracy + MAE for any mode that defined the per-frame tensors.
        if loss_type in ("discrete", "c51_asymmetric", "corn_asymmetric") and masked_correct is not None:
            metrics["masked_progress_accuracy"] = masked_correct
        if loss_type in ("discrete", "c51_asymmetric", "corn_asymmetric") and masked_mae is not None:
            metrics["masked_progress_mae"] = masked_mae
        ...
        with _timer("time/forward", timing_raw=self.timing_raw):
            if "rewind" in self.config.model.base_model_id:
                ...
                model_kwargs = {
                    ...
                    "sample_type": sample_type,
                    "timing_raw": self.timing_raw,
                    # ICL: lets the model drop demo-frame prog-token positions before the
                    # progress head runs. Set by the collator when data.use_icl=True.
                    "demo_frames_count": inputs.get("demo_frames_count", None),
                }
    ...
    def _compute_progress_loss(self, model, inputs, return_outputs=False, training=True, stratify_by_strategy=True):
        ...
        if return_outputs:
            ...
            # Per-frame ordinal MAE — bin units for discrete/c51_asymmetric, 1..5-level units for corn.
            if "masked_progress_mae" in progress_metrics:
                masked_progress_mae = progress_metrics["masked_progress_mae"]
                batch_size_m, seq_len_m = masked_progress_mae.shape
                if progress_target_mask.shape[1] != seq_len_m:
                    mae_mask_expanded = progress_target_mask.expand(batch_size_m, seq_len_m)
                else:
                    mae_mask_expanded = progress_target_mask
                progress_mae = masked_progress_mae.sum() / (mae_mask_expanded.sum() + 1e-8)
                outputs_dict[f"{prefix}/prog_ordinal_mae"] = progress_mae.item()
            ...
            if self.config.model.train_success_head:
                ...
                outputs_dict.update({
                    ...
                    f"{prefix}/success_fp_rate": success_metrics["fp_rate"].item()
                    if torch.is_tensor(success_metrics["fp_rate"])
                    else success_metrics["fp_rate"],
                    f"{prefix}/success_fn_rate": success_metrics["fn_rate"].item()
                    if torch.is_tensor(success_metrics["fn_rate"])
                    else success_metrics["fn_rate"],
                })
