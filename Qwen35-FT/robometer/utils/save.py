...
def update_cfg_with_pretrained_ckpt(
    ...
    # Replace entire model config except use_peft (keep current run's choice); only sync
    # use_multi_image / use_per_frame_progress_token for data.
    #
    # NOTE: we deliberately do NOT sync `progress_loss_type` or `progress_discrete_bins`
    # from the checkpoint any more — the Robometer-LoRA bake-off sets these via preset
    # (e.g. corn_asymmetric with a fresh 4-logit head) that the user explicitly wants to
    # override the checkpoint's original training config. Forwarding those fields from the
    # checkpoint YAML would clobber the user's choice.
    model_names = {f.name for f in fields(ModelConfig)} - {"use_peft"}
    loss_names = set()
    data_sync_fields = {"use_multi_image", "use_per_frame_progress_token"}
...
class SaveBestCallback(TrainerCallback):
    ...
    def _save_checkpoint_files(self, args: TrainingArguments, ckpt_dir: str, metrics: dict = None, step: int = None):
        ...
        if args.should_save:
            ...
            # Save metrics to JSON file (rank-0 only — avoid 8 ranks racing to write same file)
            ...
            # Save random state from training dataset (rank-0 only — random state is
            # per-rank but resume only loads one; keeping it rank-0 matches the pre-fix
            # behavior and avoids 8 ranks writing different content to the same file).
