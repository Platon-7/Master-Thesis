...
# NOTE (vendored): the originals had `from unsloth import FastVisionModel` and
# `import bitsandbytes as bnb` at module top-level. Both are unsafe to import in
# the Qwen35-FT env (transformers 5.7) — unsloth pins old transformers and bnb
# wasn't installed (it was dead code anyway: shadowed by the `bnb` parameter in
# function signatures). FastVisionModel is now lazy-imported inside
# `_load_base_model_unsloth` (only fired when cfg.use_unsloth=True, which we
# never set for full FT). bitsandbytes import dropped entirely.
...
# Qwen3.5 (transformers >= 5.7) — unified VL class, no separate Qwen3_5VLModel.
try:
...
def _is_qwen35(mid: str) -> bool:
    """Return True iff mid identifies a Qwen3.5 family model. Must be checked BEFORE
    the substring `"Qwen3" in mid` test because Qwen3 ⊂ Qwen3.5 lexically."""
...
def _load_checkpoint_weights_from_safetensors(
    ...
    # Shape-mismatch filter: torch.load_state_dict is strict about param shapes even with
    # strict=False. Two legitimate mismatches can happen for us:
    #   1. `progress_head.4.*` — Loss 1 uses progress_head_mode=corn (4 logits) vs the
    #      checkpoint's 10-bin C51 head. We want that head to random-init.
    #   2. `embed_tokens.weight` — we register an extra special token (e.g. <|demo_end|>)
    #      during setup, resizing the vocab by +1 before this load runs. The newly-added
    #      row is random-init.
    # Drop any tensor whose target shape differs from the checkpoint's, and log it.
    model_state = model.state_dict()
    skipped_mismatches: list[tuple[str, tuple, tuple]] = []
    cleaned_state_dict = {}
    for k, v in remapped_state_dict.items():
        if k in model_state and hasattr(v, "shape") and tuple(v.shape) != tuple(model_state[k].shape):
            skipped_mismatches.append((k, tuple(v.shape), tuple(model_state[k].shape)))
            continue
        cleaned_state_dict[k] = v
    if skipped_mismatches:
        logger.warning(
            f"Skipping {len(skipped_mismatches)} tensor(s) with shape mismatch (leaving them at "
            f"random init): {skipped_mismatches[:5]}"
        )
    remapped_state_dict = cleaned_state_dict
...
def _load_base_model_with_unsloth(
    ...
    # Vendored: lazy-import unsloth so the rest of setup_utils is usable in
    # the robometer_qwen35_gpu env (where unsloth is intentionally not
    # installed — incompatible with transformers 5.7).
    from unsloth import FastVisionModel  # noqa: WPS433
...
def _load_base_model_standard(
    ...
    Returns:
        Base model
    """
    # Check if it's Molmo, Qwen3.5, Qwen3 or Qwen2/2.5
    is_molmo = "Molmo" in cfg.base_model_id
    is_qwen35 = _is_qwen35(cfg.base_model_id)
    # Qwen3.5 must be excluded from the Qwen3 substring trap.
    is_qwen3 = (
        ("Qwen3" in cfg.base_model_id or "qwen3" in cfg.base_model_id.lower())
        and not is_qwen35
        and HAS_QWEN3
    )
    ...
    if is_molmo:
        ...
        if not HAS_QWEN35:
            raise RuntimeError(
                f"base_model_id={cfg.base_model_id!r} requires transformers>=5.7 "
                f"with Qwen3_5ForConditionalGeneration exported. Not found — "
                f"activate the robometer_qwen35_gpu env."
            )
        # Unified VL class for Qwen3.5; same .from_pretrained signature as the Qwen3
        # path. The hybrid linear/full attention layout is opaque at this level.
        ...
        logger.info("Using Qwen3.5 unified VL model")
    elif is_qwen3:
...
def _detect_saved_vocab_size(checkpoint_path: Optional[str]) -> Optional[int]:
    ...
    baseline into a pre-resized [N+1, D] target raises a shape mismatch. Pre-detecting
    saved vocab lets us register only N tokens before load, then add the (N+1)-th
    token post-load via resize_token_embeddings.
    """
...
def _add_special_tokens_and_resize(
    ...
    """
    # Order matters when target_vocab_size is set: tokens earlier in the list are
    # registered first. <|demo_end|> is last so baseline Robometer-4B (which lacks
    # demo_end) ends at the correct vocab.
    special_tokens = [
        "<|split_token|>",
        "<|reward_token|>",
        "<|pref_token|>",
        ...
        "<|demo_end|>",  # ICL: separator between the in-context demo trajectory and the query trajectory
    ]
...
def _verify_checkpoint_loading(cfg: ModelConfig, model: Any, before_weights: dict) -> None:
    ...
    elif _is_qwen35(cfg.base_model_id):
        # Qwen3.5: layer names confirmed only after smoke_test_04. Skip visual+layer
        # probe rather than crash; the smoke tests cover load-correctness explicitly.
        logger.info(
            "Qwen3.5 detected: skipping _verify_checkpoint_loading visual/lm probe "
            "(layer paths not yet introspected). Rely on smoke_test_03/05 instead."
        )
        return
    ...
    # vocab. With the two-stage resize fix, those shapes differ by design whenever
    # the checkpoint adds tokens (e.g., baseline Robometer-4B: N → N+1). Only run
    # allclose when shapes match — otherwise the resize itself is proof the
    # embedding was reinitialized, and torch.allclose would raise on shape diff.
...
def setup_model_and_processor(
    ...
    if "SmolVLM" in cfg.base_model_id or "Qwen" in cfg.base_model_id or "Molmo" in cfg.base_model_id:
        ...
        # When loading a checkpoint, pre-resize ONLY up to the checkpoint's saved vocab so
        # from_pretrained sees matching shapes. Any remaining tokens (e.g., <|demo_end|>
        # when loading baseline Robometer-4B) are added post-load via a second call below.
        ...
        if hf_model_id:
            ...
            if cfg.use_peft:
                ...
                if "Qwen2.5" in cfg.base_model_id:
                    ...
                    # Qwen3.5: defer architecture introspection — _verify_checkpoint_loading
                    # has a matching early-return for this family. Leave before_weights={}.
                    pass
                elif "Qwen3" in cfg.base_model_id or "Molmo" in cfg.base_model_id:
                    before_weights = {
                ...
                # the loaded rows and appends fresh init for new ones.
...
def create_training_arguments(cfg: TrainingConfig, output_dir: str, is_eval: bool = False) -> TrainingArguments:
    ...
    # transformers >= 5.7 removed several legacy TrainingArguments kwargs that the
    # original Robometer config passes (save_safetensors, possibly others). Filter
    # `base_args` to only the kwargs the installed TrainingArguments accepts so we
    # don't TypeError on first-call. This is purely defensive — every dropped key
    # is one that has a sensible default in 5.7.
    import inspect as _inspect
    _accepted = set(_inspect.signature(TrainingArguments.__init__).parameters)
    _dropped = [k for k in list(base_args) if k not in _accepted]
    for k in _dropped:
        base_args.pop(k)
    if _dropped:
        logger.info(f"create_training_arguments: dropped {_dropped} (not accepted by transformers {__import__('transformers').__version__})")
