...
# Qwen3.5 (transformers >= 5.7) is a unified VL model — no separate Qwen3_5VLModel
# class. The Qwen35-FT env imports it; the legacy robometer_gpu env (transformers
# 4.57.x) keeps Qwen3_5ForConditionalGeneration = None so any accidental dispatch
# raises a clear error rather than crashing at runtime.
try:
...
class RBM(PredictionHeadsMixin, PreTrainedModel):
    ...
    # transformers >= 5.7 renamed the support flag to `_supports_flash_attn` (no _2
    # suffix). Without it, _flash_attn_can_dispatch raises
    ...
    def __init__(self, config, processor, tokenizer, base_model=None, base_model_id=None, model_config=None):
        if "SmolVLM" in base_model_id:
            ...
            # Qwen3.5: precedence-checked BEFORE the bare "Qwen3" branch because
            # `"Qwen3" in "Qwen3.5"` would otherwise capture this id and route it
            # to Qwen3VLModel. Unified VL class on transformers >= 5.7.
            if Qwen3_5ForConditionalGeneration is None:
                raise RuntimeError(
                    f"base_model_id={base_model_id!r} requires transformers>=5.7 "
                    f"with Qwen3_5ForConditionalGeneration exported (import returned None). "
                    f"Use the robometer_qwen35_gpu env."
                )
            hidden_size = config.text_config.hidden_size
        ...
        **kwargs,
        ...
        if self.use_per_frame_progress_token:
            progress_logits, success_logits, pref_or_sim_logits = self._process_token_extraction(
                hidden_state, input_ids, sample_type, demo_frames_count=demo_frames_count
            )
    ...
    def _process_token_extraction(
        ...
        input_ids: torch.Tensor,
        sample_type: str,
        demo_frames_count: torch.Tensor | None = None,
    ) -> tuple[dict, dict, torch.Tensor | None]:
        """
        Process token-based extraction for progress and preference predictions.

        Args:
            demo_frames_count: Optional [B] int tensor from the ICL-aware collator. When non-zero
                for a given sample, the first `demo_frames_count[i]` <|prog_token|> positions
                correspond to demo (in-context) frames and must be dropped before the progress /
                success / preference heads run. This keeps head outputs aligned with query-only
                per-frame labels and preserves the even A|B split in preference batches.
        ...
        # ICL: drop leading demo prog-token hidden states per-sample so heads only see query
        # (or A/B for preference) positions. No-op when demo_frames_count is None or all zeros.
        if demo_frames_count is not None:
            sliced: list[torch.Tensor] = []
            for i, tokens in enumerate(all_prog_token_hidden):
                d = int(demo_frames_count[i].item()) if demo_frames_count.dim() > 0 else int(demo_frames_count.item())
                sliced.append(tokens[d:] if d > 0 else tokens)
            all_prog_token_hidden = sliced
    ...
    def forward(
        ...
        second_per_grid_ts=None,
        timing_raw=None,
        # transformers >= 5.7 (Qwen3.5) requires mm_token_type_ids for M-RoPE.
        # Listed explicitly here because HF Trainer's _remove_unused_columns
        # filter only keeps batch keys that appear in the named-param signature
        # (it doesn't introspect **kwargs). Without this, the column is stripped
        # from the batch and the base forward raises ValueError.
        ...
        **kwargs,
        ...
        # Pull the ICL demo-count tensor out of kwargs before dispatch so it doesn't leak into
        # the underlying VLM's forward() signature (Qwen / Molmo / SmolVLM would error on it).
        demo_frames_count = kwargs.pop("demo_frames_count", None)
        ...
        if "SmolVLM" in self.base_model_id:
            return self._forward_smolvlm(
                ...
                demo_frames_count=demo_frames_count,
                **kwargs,
            )
            ...
            qwen_kwargs = dict(kwargs)
            # Forward mm_token_type_ids (Qwen3.5 / transformers >=5.7) only when present
            # so older Qwen3-VL forwards (transformers <5.7) don't choke on the kwarg.
