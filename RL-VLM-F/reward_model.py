...
class RewardModel:
    def __init__(self, ds, da, 
        ...
        # Change: Validate input to handle actual None values and stringified nulls
        # Check if the path is a null-type string FIRST
        if cached_label_path in [None, "None", "null", ""]:
            self.cached_label_path = None
            self.all_cached_labels = []
            self.read_cache_idx = 0
        else:
            # Only format the path if it is actually a real directory
    ...
    def get_label(self, sa_t_1, sa_t_2, r_t_1, r_t_2, img_t_1=None, img_t_2=None):
        ...
        if self.vlm_label:
            ...
            good_idx = np.logical_and((vlm_labels != -1).flatten(), useful_indices)

            log_sa_t_1 = sa_t_1[log_idx]
            log_sa_t_2 = sa_t_2[log_idx]
            log_r_t_1 = r_t_1[log_idx]
            log_r_t_2 = r_t_2[log_idx]
            log_vlm_labels = vlm_labels[log_idx]
            ...
            # Change: Log VLM data for Figure
            if len(log_vlm_labels) > 0:
                log_path = os.path.join(self.log_dir, "vlm_query_log.csv")
                write_header = not os.path.exists(log_path)
                with open(log_path, "a", newline="") as log_file:
                    writer = csv.writer(log_file)
                    if write_header:
                        writer.writerow([
                            "ground_truth_score_1",
                            "ground_truth_score_2",
                            "vlm_predicted_label",
                            "timestamp",
                            "train_iter",
                        ])
                    ts = time.time()
                    for score1, score2, label in zip(
                        log_r_t_1.flatten(), log_r_t_2.flatten(), log_vlm_labels.flatten()
                    ):
                        writer.writerow([float(score1), float(score2), int(label), ts, self.train_times])
