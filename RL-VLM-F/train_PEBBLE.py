...
class Workspace(object):
    def __init__(self, cfg):
        ...
        # Initialize replay buffer and logging counters
        self.replay_buffer = ReplayBuffer(
            self.env.observation_space.shape,
            self.env.action_space.shape,
            int(cfg.replay_buffer_capacity) if not self.cfg.image_reward else 200000,  # limit image buffer size
            self.device,
    ...
    def _sync_latest_models(self, scratch_model_dir, home_model_dir, step):
        ...
        for pattern in [
            os.path.join(home_model_dir, 'actor_*.pt'),
            os.path.join(home_model_dir, 'critic_*.pt'),
            os.path.join(home_model_dir, 'critic_target_*.pt'),
            os.path.join(home_model_dir, 'reward_model_*_*.pt'),
        ]:
            ...
            # Copy current step files from scratch to home
        to_copy = []
        to_copy += glob.glob(os.path.join(scratch_model_dir, f'actor_{step}.pt'))
        to_copy += glob.glob(os.path.join(scratch_model_dir, f'critic_{step}.pt'))
        to_copy += glob.glob(os.path.join(scratch_model_dir, f'critic_target_{step}.pt'))
        to_copy += glob.glob(os.path.join(scratch_model_dir, f'reward_model_{step}_*.pt'))

        os.makedirs(home_model_dir, exist_ok=True)
        for src in to_copy:
            dst = os.path.join(home_model_dir, os.path.basename(src))
            try:
    ...
    def evaluate(self, save_additional=False):
        ...
        if self.artifact_root:
            ...
            tail = os.path.basename(self.work_dir)
            base_eval_dir = os.path.join(self.artifact_root, tail_parent, tail)
            os.makedirs(base_eval_dir, exist_ok=True)

        save_gif_dir = os.path.join(base_eval_dir, 'eval_gifs')
        ...
        for episode in range(self.cfg.num_eval_episodes):
            ...
            if save_additional:
                save_image_dir = os.path.join(base_eval_dir, 'eval_images')
                ...
                save_reward_path = os.path.join(base_eval_dir, "eval_reward")
    ...
    def run(self):
        # Home models dir (keep only latest here)
        ...
        if self.artifact_root:
            ...
            tail_parent = os.path.basename(os.path.dirname(self.work_dir))
            tail = os.path.basename(self.work_dir)
            scratch_run_dir = os.path.join(self.artifact_root, tail_parent, tail)
            scratch_model_dir = os.path.join(scratch_run_dir, "models")
        ...
        while self.step < self.cfg.num_train_steps:
            ...
            if self.step % self.cfg.save_interval == 0 and self.step > 0:
                ...
                if scratch_model_dir:
                    ...
                    # Sync only latest to home and prune old
                    self._sync_latest_models(scratch_model_dir, model_save_dir, self.step)
                else:
                    # No scratch configured: keep default behavior
