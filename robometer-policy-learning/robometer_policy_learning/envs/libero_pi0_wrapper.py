...
class LiberoPI0Wrapper(gym.Wrapper):
    ...
    def step(self, action: np.ndarray):
        ...
        # EXPERIMENT KNOB (RPL_SUPPRESS_GT_DONE=1): suppress LIBERO's ground-truth
        # success termination so episodes run to the timeout instead. Tests whether
        # the authors' RL relies on the GT `done` signal for termination/bootstrapping.
        # info["success"] is preserved above, so eval still measures success correctly
        # (evaluation_worker ORs success across steps), and the GT sparse reward is
        # left unchanged -- this isolates termination, not the reward.
        if done and os.environ.get("RPL_SUPPRESS_GT_DONE", "0") == "1":
            done = False
