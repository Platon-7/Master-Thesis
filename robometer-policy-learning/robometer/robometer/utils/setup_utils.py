...
def _add_special_tokens_and_resize(
    cfg: ModelConfig, processor: AutoProcessor, base_model: Any, checkpoint_path: str | None = None
) -> None:
    """
    ...
    Args:
        ...
        checkpoint_path: If loading a checkpoint, its path. Extra special tokens it
            carries (e.g. <|demo_end|> from demo/ICL-trained checkpoints) are added so
            the embedding size matches the checkpoint weights exactly.
    """
    ...
    # Some fine-tuned checkpoints add their own extra special tokens (e.g. <|demo_end|>
    # from demo/ICL training), which enlarges the embedding beyond the standard RBM set.
    # Add those (in checkpoint-id order) so the resized embedding matches the weights;
    # no-op for checkpoints without extras (e.g. base Robometer-4B).
    if checkpoint_path:
        try:
            ckpt_tok = AutoTokenizer.from_pretrained(checkpoint_path, trust_remote_code=cfg.trust_remote_code)
            current_vocab = processor.tokenizer.get_vocab()
            extras = sorted(
                (t for t in ckpt_tok.get_added_vocab() if t not in current_vocab),
                key=lambda t: ckpt_tok.get_added_vocab()[t],
            )
            if extras:
                processor.tokenizer.add_special_tokens({"additional_special_tokens": extras})
                logger.info(f"Added {len(extras)} extra special tokens from checkpoint: {extras}")
        except Exception as e:  # pragma: no cover - defensive
            logger.warning(f"Could not load extra special tokens from checkpoint {checkpoint_path}: {e}")
...
def setup_model_and_processor(
    ...
    if "SmolVLM" in cfg.base_model_id or "Qwen" in cfg.base_model_id or "Molmo" in cfg.base_model_id:
        ...
        _add_special_tokens_and_resize(
            cfg, processor, base_model, checkpoint_path=hf_model_id if loading_from_checkpoint else None
        )
