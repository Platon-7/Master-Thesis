...
def load_model_from_hf(
    ...
    # Each nested section (loss/model/data/...) is rebuilt via XConfig(**dict) in
    # ExperimentConfig.__post_init__, which rejects unknown keys. Checkpoints trained
    # with other robometer variants (e.g. asymmetric-loss fields: asymmetric_lambda,
    # corn_*, failure_kl_*, success_loss_type) carry extra keys this version's
    # dataclasses don't define, so drop them per-section. These are training-only and
    # irrelevant to inference / reward scoring.
    for _f in fields(ExperimentConfig):
        _val = filtered_config.get(_f.name)
        if is_dataclass(_f.type) and isinstance(_val, dict):
            _valid = {sf.name for sf in fields(_f.type)}
            filtered_config[_f.name] = {k: v for k, v in _val.items() if k in _valid}
