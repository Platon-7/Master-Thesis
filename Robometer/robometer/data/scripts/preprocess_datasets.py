...
class DatasetPreprocessor:
    ...
    def _load_dataset_from_path(self, dataset_path: str, subset: str):
        ...
        if "/" in dataset_path and not os.path.exists(dataset_path):
            ...
            # Load from local disk. `generate_hf_dataset` writes via Dataset.save_to_disk()
            # to <dataset_path>/<subset>/, producing a flat directory of arrow shards. HF's
            # load_dataset() on such a path auto-creates a "test" split and the downstream
            # code expects "train" — so we use load_from_disk and wrap in DatasetDict.
            #
            # Crucially, we also mirror the Hub branch's .map() that adds `frames_video` and
            # `frames_path` columns pointing at the real on-disk MP4 files. Without them the
            # per-example processor returns frames_processed=False and everything gets
            # filtered out in the next step.
            from datasets import load_from_disk
            local_path = os.path.join(dataset_path, subset)
            if not os.path.isdir(local_path):
                local_path = dataset_path  # fall back if the subset subdir is absent

            def _local_patch_path(old_path):
                # `frames` stores a path relative to dataset_path (see generate_hf_dataset's
                # process_single_trajectory: relative_video_path = dataset_name/subdir/traj.mp4).
                return os.path.join(dataset_path, old_path)

            ds = load_from_disk(local_path)
            if isinstance(ds, DatasetDict):
                mapped = {k: v.map(
                    lambda x: {"frames_video": _local_patch_path(x["frames"]),
                               "frames_path":  _local_patch_path(x["frames"])}
                ) for k, v in ds.items()}
