...
def _extract_trajectory_rewards(
    ...
    # CORN head (losses.md §Loss 1) emits 4 cumulative-threshold logits per frame, so
    # `pred_array` arrives shape [T, 4] instead of [T]. Decode to scalar per-frame expected
    # progress in [0, 1] before any aggregation: E[y] − 1 = Σ_k σ(z_k), normalised by 4.
    # Without this the "last" aggregation tries `float(pred_array[-1])` on a 4-vector and
    # raises `TypeError: only length-1 arrays can be converted to Python scalars`.
    if pred_array.ndim == 2 and pred_array.shape[-1] == 4:
        sigma = 1.0 / (1.0 + np.exp(-pred_array))
        pred_array = sigma.sum(axis=-1) / 4.0
