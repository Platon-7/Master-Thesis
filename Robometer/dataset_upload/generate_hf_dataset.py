...
class DatasetConfig:
    ...
    # Optional knobs read by the robometer_frames_* loader (Robometer-LoRA's keyframe dataset).
    # Ignored by every other loader.
    family: Optional[str] = field(
        default=None,
        metadata={"help": "robometer_frames_* loader: which family subdir to load "
                  "(metaworld | failsafe | droid | robometer | roboreward)."},
    )
    include_failures: bool = field(
        default=True, metadata={"help": "robometer_frames_* loader: include failure manifests."}
    )
    include_successes: bool = field(
        default=True, metadata={"help": "robometer_frames_* loader: include success manifests."}
    )
    max_episodes_per_archive: Optional[int] = field(
        default=None,
        metadata={"help": "robometer_frames_* loader: cap episodes per shard archive (None = no cap)."},
    )
    episode_ids_filter: Optional[str] = field(
        default=None,
        metadata={"help": "robometer_frames_* loader: path to a JSONL produced by build_splits.py "
                  "to restrict the loader to a specific train/eval split."},
    )
    pairs_index_path: Optional[str] = field(
        default=None,
        metadata={"help": "robometer_frames_* loader: path to a build_splits.py-produced "
                  "pairs_index_<split>.jsonl. When set, the multi-family split-aware loader is used "
                  "(recommended for our train + eval slices since they span multiple families)."},
    )
...
def main(cfg: GenerateConfig):
    ...
    if cfg.dataset.dataset_name and "robometer_frames" in cfg.dataset.dataset_name:
        ...
        from robometer_frames_loader import (
            load_robometer_frames_dataset,
            load_robometer_frames_split,
        )
        ...
        if cfg.dataset.pairs_index_path:
            ...
            if not family:
                # Infer family from the dataset name suffix:
                # robometer_frames_metaworld → metaworld, robometer_frames_eval_droid → droid, etc.
                tail = cfg.dataset.dataset_name.split("robometer_frames_", 1)[-1]
                for prefix in ("eval_", "train_"):
                    if tail.startswith(prefix):
                        tail = tail[len(prefix):]
                        break
                family = tail
            task_data = load_robometer_frames_dataset(
                base_path=cfg.dataset.dataset_path,
                family=family,
                include_successes=cfg.dataset.include_successes,
                include_failures=cfg.dataset.include_failures,
                max_episodes_per_archive=cfg.dataset.max_episodes_per_archive,
                episode_ids_filter=cfg.dataset.episode_ids_filter,
            )
