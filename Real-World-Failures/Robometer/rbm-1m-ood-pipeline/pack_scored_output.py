...
def build_meta(episode_id: str, archive: str,
    ...
    # rbm-1m-ood pipeline post-processes scoring output to embed the per-frame
    # ordinal labels + trajectory-level terminal_reward + pairing info directly,
    # matching the meta.json schema in the existing robometer_frame_dataset shards.
    for k in ("terminal_reward", "frame_labels", "paired_success_id"):
        if k in s:
            meta[k] = s[k]
    if "keyframes_dir" in m:
        meta["keyframes_dir"] = m["keyframes_dir"]
