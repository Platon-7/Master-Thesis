...
def main(cfg: OmegaConf):
    ...
    # Determine VLM device from config (auto = cuda:1 if 2+ GPUs, else cuda:0)
    ...
    if use_vlm_reward:
        print("\n" + "="*60)
        print("DSRL + RoboReward VLM Training")
        print("="*60)
        print(f"   Policy device ({cfg.device}): DSRL (Diffusion + SAC)")
        print(f"   VLM device ({vlm_device}): RoboReward-8B")
        print("="*60)
        ...
        print("\nRunning with SIMULATOR rewards (VLM disabled)\n")
    ...
    # Evaluation env uses SIMULATOR rewards (ground-truth) for trustworthy metrics
