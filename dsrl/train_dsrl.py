...
import gym
# d4rl is only needed for Gym environments (hopper, walker2d, halfcheetah), not Robomimic
try:
    import d4rl
    import d4rl.gym_mujoco
except ImportError:
    d4rl = None  # Will fail later if trying to use Gym environments without d4rl
import sys
...
def main(cfg: OmegaConf):
	...
	# Train the agent
	total_timesteps = cfg.get('total_timesteps', 20000000)  # Default to 20M if not specified
	model.learn(
