#!/usr/bin/env python3
"""
FPS Sensitivity Analysis for RoboReward Model
=============================================

This script systematically evaluates the RoboReward VLM across different FPS settings
and generates confusion matrices to analyze classification performance.

Key Finding: The qwen_vl_utils library defaults to FPS=2.0 when no fps is specified.
This can significantly impact model predictions on robotics videos.
...
class FPSExperiment:
    ...
    def run(self) -> Dict[str, FPSExperimentResult]:
        ...
        print("\n" + "="*80)
        print(f"🔬 FPS SENSITIVITY EXPERIMENT")
        print(f"   Dataset: {self.dataset.name}")
        print(f"   Task: {self.dataset.task_instruction}")
        print("="*80)
    ...
    def save_results(self, filename: Optional[str] = None):
        ...
        if filename is None:
            ...
            filename = f"fps_experiment_{self.dataset.name}_{timestamp}.json"
