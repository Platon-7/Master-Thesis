...
class MainConfig(common_utils.RunConfig):
    ...
    # log
    use_wb: int = 1
    save_dir: str = "exps/rl/metaworld/vlm_critic-run1"
    train_end_on_success: int = 1

    # For VLMEnv
    vlm: str = "demo2reward_qwen3_8b"
    past_len: int = 4
    reward_at_truncation: int = 0
    # Where the 224x224 GVL demo dataset lives. Only used when vlm starts with "gvl_".
...
class Workspace:
    def __init__(self, cfg: MainConfig):
        ...
        self.end_on_success = cfg.train_end_on_success
    ...
    def _setup_env(self):
        ...
        self.env_params["end_on_success"] = bool(self.end_on_success)
        self.env_params["episode_length"] = self.cfg.episode_length
        self.env_params["env_reward_scale"] = self.cfg.env_reward_scale
        # Optional: score the VLM reward from a different (legible) camera than the
        # policy's rl_camera. The policy keeps its zoomed corner2 obs; the reward model
        # sees ROBOMETER_REWARD_CAMERA (e.g. topview). Rendering an extra camera only
        # adds a key to image_obs — rl_obs (policy input) is unchanged.
        reward_cam = os.environ.get("ROBOMETER_REWARD_CAMERA", "").strip()
        if reward_cam and reward_cam not in self.env_params.get("camera_names", []):
            self.env_params["camera_names"] = list(self.env_params["camera_names"]) + [reward_cam]
            print(f"[reward-camera] scoring VLM reward from '{reward_cam}'; camera_names={self.env_params['camera_names']}")
        self.train_env = VLMCritic_PixelMetaWorld(
            **self.env_params,
            vlm=self.cfg.vlm,
            ...
            reward_camera=(reward_cam or self.env_params["rl_camera"]),
        )

        eval_env_params = self.env_params.copy()
        eval_env_params["env_reward_scale"] = 1.0
        eval_env_params["end_on_success"] = True
        ...
        while self.global_step < self.cfg.num_train_step:
            ...
            if terminal:
                with stopwatch.time("reset"):
                    ...
                    stat["score/train_return"].append(sum_rewards)
                    stat["score/train_mean_reward"].append(sum_rewards/self.train_env.time_step)
                    stat["score/train_score"].append(success)
                    stat["data/episode_len"].append(self.train_env.time_step)

                    episode_info = self.train_env.episode_stats
                    assert 'vlm_reward_counts' in episode_info
                    vlm_reward_counts = episode_info['vlm_reward_counts']
                    stat[f"data/vlm_reward_counts"].append(vlm_reward_counts)
                    stat[f"data/early_termination"].append(episode_info['early_termination'])
