...
class RobometerScorer:
    ...
    def __init__(self, model_path: str, device: str = "cuda", max_frames: int = None):
        ...
        # variant, not just the literal "discrete": FT checkpoints use loss types
        # like "c51_asymmetric" (same 10-bin head); matching only "discrete" left
        # their reads on the raw-logit path -> garbage progress.
    ...
    def __call__(self, frames: Sequence[_FrameLike], task: str, episode_id: int = 0,
        ...
        # readout so callers can apply alternative decodes (condMean etc.).
